...
\centering
\vspace*{2cm}
{\Large TRABAJO FINAL DE CARRERA}\\[1.5cm]
{\LARGE\bfseries Automatización de Gestión Empresarial\\ con n8n: Asistente Inteligente para\\ Taller Mecánico vía Telegram}\\[1cm]
{\large Línea de Investigación: Workflows Aplicados a la Gestión Empresarial y Marketing}\\[1.5cm]
{\large Tecnicatura Universitaria en Programación}\\[0.5cm]
{\large Universidad Tecnológica Nacional}\\[0.5cm]
{\large Mendoza, 2026}\\[1cm]
{\large Profesor: Cortez, Alberto Alejandro}\\[0.5cm]
{\large Estudiantes: Palmero, Manuel; Rojas, Uriel}\\[0.5cm]
\end{titlepage}
...
\begin{enumerate}
  \item Diseñar e implementar un agente de inteligencia artificial de atención al cliente capaz de interpretar peticiones en lenguaje natural para registrar turnos, incorporar nuevos clientes y consultar la disponibilidad de repuestos.
  \item Implementar un agente de gestión administrativa con acceso restringido que permita consultar el inventario completo, actualizar el stock y acceder a la lista consolidada de turnos y clientes.
  \item Establecer la integración entre n8n y Google Sheets como repositorio centralizado de datos, garantizando la integridad de la información mediante autenticación OAuth2.
  \item Configurar el modelo Mistral Large como motor de razonamiento de ambos agentes mediante los nodos nativos de n8n para LangChain.
  \item Desarrollar una lógica de enrutamiento que diferencie los canales de acceso público y privado mediante la validación del identificador de usuario de Telegram (User ID) contra una lista de identificadores de administradores autorizados, almacenando el resultado en una variable booleana denominada \texttt{admin} que determina el flujo de ejecución a través de un nodo Switch.
  \item Validar el sistema mediante un protocolo de pruebas funcionales unitarias, de integración y de seguridad, documentando los resultados con evidencia empírica.
\end{enumerate}
...
\section{Automatización de Procesos de Negocio (BPA/RPA)}
...
\section{Plataformas Low-Code y n8n}
...
\section{Agentes Conversacionales basados en LLM}
...
\section{Integración del Framework LangChain}
...
Un componente técnico central en la integración de LangChain con Google Sheets dentro de n8n es la expresión \texttt{\$fromAI()}. Esta expresión actúa como marcador dinámico en la configuración de los nodos herramienta: cuando el agente LLM decide invocar una herramienta, n8n reemplaza en tiempo de ejecución cada \texttt{\$fromAI()} por el valor que el modelo de lenguaje determina apropiado para ese parámetro en función del contexto conversacional. Esto permite, por ejemplo, que el agente extraiga el DNI del cliente directamente del texto del mensaje y lo inyecte como filtro en la consulta a Google Sheets sin que el desarrollador deba programar dicha extracción de manera explícita.

\section{Modelos de Lenguaje de Gran Escala: Mistral AI}
...
En el workflow desarrollado, el modelo se integra mediante el nodo Mistral Cloud Chat Model configurado con la variante \texttt{mistral-large-latest}, que recibe el prompt de sistema, el historial de conversación y las descripciones de las herramientas disponibles, devolviendo la decisión sobre la acción a ejecutar. La arquitectura instancia dos nodos separados, uno por agente, lo que permite parametrización independiente de la temperatura y eventual diferenciación del modelo subyacente en futuras iteraciones.

\section{Persistencia de Datos en Hojas de Cálculo Conectadas}
...
Las limitaciones de este enfoque son reconocidas en la sección \ref{sec:limitaciones} y abordadas en las líneas de trabajo futuro: ausencia de transacciones ACID, riesgo de condiciones de carrera bajo concurrencia y techo de escalabilidad que impone la API de Google Sheets.

\section{Telegram Bot API como Interfaz Conversacional}
...
La revisión de antecedentes se realizó mediante búsqueda en las bases Google Scholar, Semantic Scholar y Redalyc, utilizando los descriptores \textit{chatbot small business automation}, \textit{LLM business process automation}, \textit{n8n workflow automation}, \textit{conversational agent appointment scheduling}, \textit{PyME automatización taller mecánico} y sus combinaciones en inglés y español. Se priorizaron publicaciones de los últimos cinco años (2020--2025).

\subsection{Automatización conversacional y agentes con uso de herramientas}
...
\subsection{Digitalización de PyMEs en el sector automotriz y de servicios}
...
\section{Tipo y Enfoque de Investigación}
...
\section{Diseño de Investigación: Estudio de Caso}

Se adopta el diseño de estudio de caso instrumental único (Stake, 1995), entendido como el examen detallado de una situación particular con el propósito de iluminar un fenómeno más amplio. El caso seleccionado es un taller mecánico de pequeña escala ubicado en el Gran Mendoza, cuyas características operativas se describen en la sección \ref{sec:caracterizacion}.
...
\section{Procedimiento de Desarrollo Iterativo}

El desarrollo del sistema siguió un modelo iterativo e incremental, organizado en cinco iteraciones documentadas:

\begin{enumerate}
  \item \textbf{Iteración 1}: Flujo mínimo viable con trigger de Telegram, agente único de servicio al cliente y herramienta de registro de turnos.
  \item \textbf{Iteración 2}: Verificación de cliente existente por DNI y registro condicional. Corrección de errores en inyección dinámica mediante \texttt{\$fromAI()}.
  \item \textbf{Iteración 3}: Segundo agente y lógica de enrutamiento mediante nodo Switch.
  \item \textbf{Iteración 4}: Mecanismo de confirmación explícita para modificación de stock y subworkflow de verificación de disponibilidad.
  \item \textbf{Iteración 5}: Refinamiento de prompts, ajuste de formato de respuesta para Telegram y pruebas integrales.
\end{enumerate}

\section{Técnicas de Análisis y Validación}

La validación funcional del sistema se diseñó como un protocolo de pruebas estructurado en tres niveles: pruebas unitarias (verificación aislada de cada herramienta), pruebas de integración (flujos conversacionales completos) y pruebas de enrutamiento y seguridad (discriminación de roles). Los resultados completos se presentan en el Capítulo \ref{cap:resultados}.

% =================================================================
\chapter{Diseño de la Solución}
% =================================================================

\section{Arquitectura General del Workflow}
...
\begin{enumerate}
  \item \textbf{Capa de entrada}: Nodo Telegram Trigger, configurado para escuchar eventos de tipo \texttt{message}.
  \item \textbf{Capa de validación de acceso}: Nodo de procesamiento (Code) que extrae el campo \texttt{from.id}, lo compara contra una lista de administradores autorizados y establece la variable booleana \texttt{admin}.
  \item \textbf{Capa de enrutamiento}: Nodo Switch que evalúa la variable \texttt{admin} y el contenido del mensaje para dirigir a una de tres rutas.
  \item \textbf{Capa de agentes}: Dos ramas independientes, cada una con su nodo AI Agent (tipo Tools Agent), su instancia dedicada de Mistral Cloud Chat Model y su memoria Window Buffer.
  \item \textbf{Capa de datos}: Nodos Google Sheets Tool distribuidos entre los agentes según el principio de privilegio mínimo, más un subworkflow de consulta de disponibilidad.
\end{enumerate}

\section{Lógica de Enrutamiento}
\label{sec:enrutamiento}

El workflow incorpora un nodo de procesamiento inmediatamente posterior al Telegram Trigger que extrae el campo \texttt{from.id} del objeto de mensaje, correspondiente al identificador numérico único del usuario emisor en la plataforma Telegram. Dicho identificador se compara contra una lista de identificadores de administradores autorizados, cargada manualmente como parámetro de configuración del workflow. El resultado de esta comparación se almacena en una variable booleana denominada \texttt{admin}, cuyo valor es verdadero si el User ID del emisor figura en la lista de autorizados, y falso en caso contrario.
...
\midrule
\texttt{admin} es verdadera & Agente Administrativo & Acceso directo al panel \\
\addlinespace
Texto igual a \texttt{"/start"} y \texttt{admin} es falsa & Mensaje de Bienvenida & Mensaje estático \\
\addlinespace
...
\section{Agente de Servicio al Cliente (Público)}
\label{sec:agente-cliente}
...
\begin{itemize}
  \item \texttt{Consultar\_Stock\_Publico1}: Get Many sobre la hoja Stock, sin columnas de costo.
  \item \texttt{Obtener Clientes}: Get Many sobre la hoja Clientes (uso exclusivo para cálculo de IDs).
  \item \texttt{Obtener El ID por DNI}: Get by Filter sobre Clientes filtrando por DNI.
  \item \texttt{Herramienta\_Verificar\_DNI1}: Get by Filter sobre Clientes filtrando por DNI (verificación redundante).
  \item \texttt{Registrar Cliente1}: Append Row sobre la hoja Clientes.
  \item \texttt{Obtener Turnos}: Get Many sobre la hoja Turnos (uso exclusivo para cálculo de IDs).
  \item \texttt{Obtener Turno Por id}: Get by Filter sobre Turnos filtrando por ID\_Cliente.
  \item \texttt{Agendar\_Turno}: Append Row sobre la hoja Turnos.
  \item \texttt{Herramienta\_Cancelar\_Turno1}: Update Row sobre Turnos (cambio de estado a Cancelado).
  \item \texttt{Modificar\_turno}: Update Row sobre Turnos (reprogramación de turno).
  \item \texttt{Consultar Disponibilidad Tool}: Workflow Tool que invoca un subworkflow dedicado para verificar disponibilidad de turnos.
\end{itemize}

\subsection{Subworkflow de Consulta de Disponibilidad}

El sistema incorpora un subworkflow independiente denominado \texttt{SubWorkflow\_Consultar\_Disponibilidad}, invocado como herramienta por el agente público. Este subworkflow recibe dos parámetros (\texttt{fecha} y \texttt{horario}), consulta la hoja Turnos de Google Sheets y determina si el horario solicitado está disponible. La lógica de verificación excluye los turnos con estado \texttt{Cancelado} para evitar falsos positivos. Retorna \texttt{LIBRE} o \texttt{OCUPADO} según el resultado.

\section{Agente Administrativo (Privado)}

El Agente Administrativo está protegido por el mecanismo de validación de User ID descrito en la sección \ref{sec:enrutamiento}. Solo los usuarios cuyo identificador de Telegram figure en la lista de administradores autorizados son enrutados hacia este agente. El agente administrativo dispone de un conjunto de herramientas que incluyen capacidades de modificación de datos:

\begin{itemize}
  \item \texttt{Obtener Repuestos}: Get Many sobre la hoja Stock, con acceso completo a todas las columnas.
  \item \texttt{Obtener Clientes}: Get Many sobre la hoja Clientes (lista completa).
  \item \texttt{Obtener Turnos}: Get Many sobre la hoja Turnos (todos los registros).
  \item \texttt{Actualizar Stock1}: Update Row sobre la hoja Stock.
\end{itemize}

\section{Mecanismo de Seguridad y Confirmación}

El sistema implementa dos niveles de seguridad diferenciados. El primer nivel opera sobre el enrutamiento: antes de que cualquier mensaje alcance al Agente Administrativo, el workflow extrae el User ID del emisor desde el campo \texttt{from.id} del objeto de actualización de Telegram y lo compara contra una lista de identificadores de administradores autorizados. Esta comparación se realiza mediante un nodo de procesamiento previo al Switch, que establece la variable booleana \texttt{admin} como verdadera únicamente si el identificador figura en la lista. El nodo Switch evalúa esta variable y deriva el flujo hacia el Agente Administrativo solo cuando \texttt{admin} es verdadera. Este mecanismo impide que el contenido del mensaje pueda utilizarse para manipular el enrutamiento.

El segundo nivel opera sobre las operaciones de modificación de datos: el prompt de sistema del Agente Administrativo incluye una instrucción de confirmación explícita previa a la ejecución de \texttt{Actualizar Stock1}. El agente debe consultar al usuario antes de invocar la herramienta y ejecutarla únicamente ante una respuesta afirmativa explícita.

\section{Estructura de la Base de Datos}
\label{sec:schema}

El documento \texttt{Taller mecánico} en Google Sheets se estructura en tres hojas vinculadas relacionalmente.

\subsection*{Hoja Stock}
...
\subsection*{Hoja Clientes}

\begin{tabular}{lll}
\toprule
Columna & Tipo & Descripción \\
\midrule
ID\_Cliente & Numérico & Identificador único del cliente \\
...
\subsection*{Hoja Turnos}

\begin{tabular}{lll}
\toprule
Columna & Tipo & Descripción \\
\midrule
ID\_Turno & Numérico & Identificador único del turno \\
ID\_Cliente & Numérico & Clave foránea hacia Clientes \\
...
\section{Limitaciones Técnicas Reconocidas}
...
El bot se creó mediante BotFather, obteniendo el token de autenticación que se almacenó en las credenciales de n8n. El nodo Telegram Trigger se configuró para recibir actualizaciones de tipo \texttt{message}. La validación de administradores se implementa en un nodo Code inmediatamente posterior al Telegram Trigger: extrae el campo \texttt{from.id}, lo compara contra la lista de User IDs autorizados y establece la variable booleana \texttt{admin}.

\section{Configuración de los Agentes de IA}

Cada agente se configuró con un prompt de sistema detallado (incluido en el Anexo \ref{anexo:prompts}) que define rol, capacidades, flujos de trabajo y reglas de comportamiento. Se utilizó el tipo Tools Agent de LangChain con modo de prompt define. La memoria se implementó mediante nodos Window Buffer Memory con ventana de 30 mensajes. Para el agente de clientes, la clave de sesión se compone del identificador de chat (\texttt{chat.id}) con un sufijo de versión, permitiendo aislamiento de contexto por usuario.

\section{Integración con Google Sheets}

La integración se realizó mediante autenticación OAuth2. Cada nodo Google Sheets Tool se configuró con el ID del documento \texttt{Taller mecánico} y el nombre de la hoja correspondiente. Para la búsqueda de clientes por DNI se utilizó el modo Get by Filter, configurando la columna DNI como clave de búsqueda con valor inyectado dinámicamente mediante la expresión \texttt{\$fromAI()}.

\section{Configuración del Modelo Mistral}

Se instanciaron dos nodos Mistral Cloud Chat Model independientes utilizando la variante \texttt{mistral-large-latest}, uno por agente, garantizando aislamiento contextual y permitiendo parametrización diferenciada. Ambos nodos comparten el token de API pero operan en pipelines de ejecución separados. La temperatura se fijó en 0.3 tras una serie de pruebas comparativas informales con valores de 0.1, 0.3 y 0.7.

\section{Subworkflow de Consulta de Disponibilidad}

El subworkflow independiente recibe dos parámetros de entrada (\texttt{fecha} y \texttt{horario}), lee la hoja Turnos mediante un nodo Google Sheets en modo Get Many, y procesa los resultados con un nodo Code que verifica si existe un turno no cancelado en la fecha y horario solicitados. Retorna un objeto JSON con los campos \texttt{result} (LIBRE u OCUPADO) y \texttt{disponibilidad} (booleano).

\section{Mecanismo de Respuesta}
...
% =================================================================
\chapter{Resultados y Validación Funcional}
\label{cap:resultados}
% =================================================================

\section{Diseño Experimental de las Pruebas}
...
\section{Resultados de las Pruebas de Enrutamiento y Seguridad}
...
\begin{enumerate}
  \item Refuerzo del mecanismo de autenticación administrativa, sustituyendo la lista estática por un sistema dinámico.
  \item Implementación de persistencia de memoria conversacional con Redis o almacén de sesiones dedicado.
  \item Migración de la persistencia a un sistema de gestión de bases de datos relacional (PostgreSQL o Supabase).
  \item Integración con Google Calendar para gestión dinámica de disponibilidad.
  \item Incorporación de recordatorios automatizados mediante workflows programados.
\end{enumerate}
...
\item Adamopoulou, E., \& Moussiades, L. (2020). Chatbots: History, technology, and applications. \textit{Machine Learning with Applications}, 2, 100006.
\item Chase, H. (2022). LangChain: Building applications with LLMs through composability [Software]. https://github.com/langchain-ai/langchain
\item Eller, R., Alford, P., Kallmünzer, A., \& Peters, M. (2020). Antecedents, consequences, and challenges of small and medium-sized enterprise digitalization. \textit{Journal of Business Research}, 112, 119--127.
\item Hernández-Sampieri, R., \& Mendoza, C. P. (2018). \textit{Metodología de la investigación: Las rutas cuantitativa, cualitativa y mixta}. McGraw-Hill.
\item Hofmann, P., Samp, C., \& Urbach, N. (2020). Robotic process automation. \textit{Electronic Markets}, 30(1), 99--106.
\item Jiang, A. Q., et al. (2023). Mistral 7B. arXiv. https://doi.org/10.48550/arXiv.2310.06825
\item Sahay, A., Indamutsa, A., Di Ruscio, D., \& Pierantonio, A. (2020). Supporting the understanding and comparison of low-code development platforms. \textit{Proceedings of the 46th Euromicro Conference on Software Engineering and Advanced Applications}, 171--178.
\item Stake, R. E. (1995). \textit{The art of case study research}. Sage Publications.
\item Statista. (2025). Most popular social media platforms in Argentina as of 2nd quarter 2025. https://www.statista.com/
\item Syed, R., et al. (2020). Robotic process automation: Contemporary themes and challenges. \textit{Computers in Industry}, 115, 103162.
\item van der Aalst, W. M. P., Bichler, M., \& Heinzl, A. (2018). Robotic process automation. \textit{Business \& Information Systems Engineering}, 60(4), 269--272.
\item Yao, S., et al. (2023). ReAct: Synergizing reasoning and acting in language models. \textit{Proceedings of the 11th ICLR}.
\end{enumerate}

\noindent\textbf{Documentación técnica consultada}

\begin{itemize}
  \item LangChain. (2025). LangChain documentation: Agents. https://python.langchain.com/docs/modules/agents/
  \item Mistral AI. (2025). Mistral AI API documentation. https://docs.mistral.ai/
  \item n8n. (2025). n8n documentation. https://docs.n8n.io/
  \item Telegram. (2025). Telegram Bot API. https://core.telegram.org/bots/api
\end{itemize}
...
\section*{Anexo 8.1. Prompts de Sistema Completos}
\label{anexo:prompts}
\addcontentsline{toc}{section}{Anexo 8.1. Prompts de Sistema Completos}

\subsection*{8.1.1. Prompt del Agente Público (Servicio Técnico)}
\addcontentsline{toc}{subsection}{8.1.1. Prompt del Agente Público}

El prompt del agente público es extenso e incluye las siguientes secciones principales:

\begin{enumerate}
  \item \textbf{Rol e identidad}: Se define como asistente virtual del Taller Mecánico, en español rioplatense (voseo), con mensajes breves y claros.
  \item \textbf{Contexto temporal y calendario}: Incluye la fecha y hora actual mediante expresiones \texttt{\$now} de n8n, con una tabla de fechas relativas (mañana, en 2 días, etc.) y reglas de calendario (domingos cerrado, horario 08:00--18:00).
  \item \textbf{Regla de prioridad -- disponibilidad}: Ante cualquier mención de horario específico, debe consultar disponibilidad con la herramienta \texttt{consultar\_disponibilidad} antes de solicitar datos personales.
  \item \textbf{Descripción detallada de cada herramienta}: Con nombres exactos, parámetros esperados y ejemplos de uso.
  \item \textbf{Regla crítica de IDs incrementales}: Procedimiento paso a paso para calcular ID\_Cliente e ID\_Turno como máximo existente + 1.
  \item \textbf{Flujo de agendamiento de turno}: 7 pasos secuenciales desde verificación de disponibilidad hasta confirmación.
  \item \textbf{Flujo de modificación/reprogramación}: Validación de horario, verificación de disponibilidad y confirmación.
  \item \textbf{Flujo de cancelación}: Identificación, verificación y confirmación.
  \item \textbf{Reglas generales}: Prohibición de inventar valores, un tema por vez, confirmación explícita siempre.
\end{enumerate}

\subsection*{8.1.2. Prompt del Agente Privado (Panel Administrativo)}
\addcontentsline{toc}{subsection}{8.1.2. Prompt del Agente Privado}
...
\section*{Anexo 8.2. Diagrama de Flujo del Workflow Principal}
\addcontentsline{toc}{section}{Anexo 8.2. Diagrama de Flujo del Workflow Principal}

Descripción estructurada del flujo principal:

El workflow se inicia con el nodo \textbf{TELEGRAM TRIGGER} (evento: message). Inmediatamente después, un nodo \textbf{CODE} extrae el campo \texttt{from.id} y lo compara contra la lista de administradores autorizados, estableciendo la variable \texttt{admin}.

El nodo \textbf{SWITCH} evalúa en orden:

\begin{enumerate}
  \item \textbf{Condición}: \texttt{admin} es verdadera $\rightarrow$ \textbf{Agente Administrativo}
        \begin{itemize}
          \item AI AGENT (Agente Administrativo)
          \item Tools: Obtener Repuestos, Obtener Clientes, Obtener Turnos, Actualizar Stock1
          \item Salida: Telegram Send Message
        \end{itemize}
  \item \textbf{Condición}: \texttt{admin} es falsa y texto es \texttt{"/start"} $\rightarrow$ \textbf{Mensaje de Bienvenida}
        \begin{itemize}
          \item Telegram Send Message (texto estático)
        \end{itemize}
  \item \textbf{Default}: \textbf{Agente Público}
        \begin{itemize}
          \item AI AGENT (Agente Público)
          \item Tools: Consultar\_Stock\_Publico1, Obtener El ID por DNI, Herramienta\_Verificar\_DNI1, Obtener Clientes, Obtener Turnos, Obtener Turno Por id, Registrar Cliente1, Agendar\_Turno, Herramienta\_Cancelar\_Turno1, Modificar\_turno, Consultar Disponibilidad Tool
          \item Memoria: Window Buffer (30 mensajes, clave por chat.id)
          \item Salida: Telegram Send Message
        \end{itemize}
\end{enumerate}

\section*{Anexo 8.3. Modelo de Datos Lógico}
\addcontentsline{toc}{section}{Anexo 8.3. Modelo de Datos Lógico}

Entidades y relaciones:

\begin{itemize}
  \item \textbf{Cliente} (\underline{ID\_Cliente} PK, Nombre y Apellido, DNI UK, Dirección, Número De Teléfono)
  \item \textbf{Turno} (\underline{ID\_Turno} PK, \underline{ID\_Cliente} FK, Auto, Motivo, Fecha del Turno, Horario, Estado)
  \item \textbf{Repuesto} (\underline{Código} PK, Repuesto, Stock Actual, Stock Mínimo, Precio Unitario, Categoría)
\end{itemize}

Relaciones: un Cliente puede tener muchos Turnos (1:N) mediante ID\_Cliente. Los IDs son numéricos e incrementales, gestionados por el agente mediante consulta del máximo valor existente.

\section*{Anexo 8.4. Guion de Entrevista al Caso de Estudio}
\label{anexo:entrevista}
\addcontentsline{toc}{section}{Anexo 8.4. Guion de Entrevista al Caso de Estudio}

\begin{itemize}
  \item \textbf{Bloque 1 -- Gestión de turnos}: ¿Cómo registra los turnos actualmente? ¿Cuántos turnos se solapan o duplican por mes? ¿Cuántas consultas recibe fuera del horario laboral?
  \item \textbf{Bloque 2 -- Control de inventario}: ¿Cómo lleva el stock de repuestos? ¿Con qué frecuencia detecta faltantes durante una reparación? ¿Cuánto tiempo le insume verificar el stock?
  \item \textbf{Bloque 3 -- Comunicación con clientes}: ¿Por qué canales se comunican los clientes? ¿Conserva historial de servicios por cliente? ¿Cómo gestiona los recordatorios?
  \item \textbf{Bloque 4 -- Disposición tecnológica}: ¿Qué herramientas digitales utiliza actualmente? ¿Qué barreras identifica para adoptar nuevas herramientas? ¿Estaría dispuesto a probar un asistente automatizado?
\end{itemize}

\section*{Anexo 8.5. Capturas de Pantalla de Pruebas Funcionales}
\addcontentsline{toc}{section}{Anexo 8.5. Capturas de Pantalla de Pruebas Funcionales}
